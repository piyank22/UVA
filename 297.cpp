...
void build( int s , int e , int x , int y )
{
	pos++;
	if(str[pos]=='p')
	{
		build( s , 1+(e+y)/2 , (s+x)/2 , y );
		build( s , e , (s+x)/2 , (e+y)/2 );
		build( 1+(s+x)/2 , e , x , (e+y)/2 );
		build( 1+(s+x)/2 , 1+(e+y)/2 , x , y );
	}
	else if(str[pos]=='f')
		forr(i,s,x)	forr(j,e,y) pixel[i][j] = 1;
}
...
int main()
{
	test_case
	{
		...
		forr(i,1,32)	forr(j,1,32) 	ans+=pixel[i][j];
