...
int main()
{
  while( cin >> n >> m , n|m )
  {
    ...
    forr(i,1,n) forr(j,1,m)
    {
      double dd;
      cin >> dd;
      g.add_edge( i , n+j , 1 , dd );
    }
    forr(i,1,n) g.add_edge( 0 , i , 1 , 0 );
    forr(i,1,m) g.add_edge( n+i , n+m+1 , 1 , 0 );

    double ans = g.min_cost_max_flow(0,n+m+1).snd / n ;
    ans = floor(ans*100.0+0.5+1e-9)/100.;
