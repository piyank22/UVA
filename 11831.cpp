...
{   
    ...
    while( cin >> n >> m >> s , n|m|s )
    {
        ...
        if( arena[row][col] == 'N')
            dir = 0;
        else if( arena[row][col] == 'S' )
            dir = 2;
        else if( arena[row][col] == 'L' )
            dir = 1;
        else 
            dir = 3;
        ...
        forr(i,0,s-1)
        {
            char c = sequence[i];
            if( c == 'D' )          dir = (dir+1)%4;
            else if( c == 'E' )     dir = (dir+3)%4;
            else
            {
                int rr = row+x[dir];
                int cc = col+y[dir];
                if( rr<0 || rr>=n || cc<0 || cc>=m || arena[rr][cc]=='#' )  continue;
                if( arena[rr][cc]=='*' )
                {
                    ans++;
                    arena[rr][cc]='.';
