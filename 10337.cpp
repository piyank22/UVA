...
int solve( int pos , int height )
{
    ...
    int &ret = memo[pos][height];
    if( ~ret )  return ret;

    ret = 1e9;
    ret = min( ret , 60 - arr[height][pos] + solve(pos+1,height+1) );
    ret = min( ret , 30 - arr[height][pos] + solve(pos+1,height+0) );
    ret = min( ret , 20 - arr[height][pos] + solve(pos+1,height-1) );
    return ret;
}
