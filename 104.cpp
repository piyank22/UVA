    ...
    while( cin >> n )
    {
        ...
        forr(steps,2,n)
            forr(k,0,n-1)
                forr(i,0,n-1)
                    forr(j,0,n-1)
                        if( mat[i][j][steps] < mat[i][k][steps-1] * mat[k][j][1] )
                        {
                            mat[i][j][steps] = mat[i][k][steps-1] * mat[k][j][1];
                            parent[i][j][steps] = k;
                        }
        ...
        forr(steps,2,n)
        {
            forr(i,0,n-1)
            {
                if( mat[i][i][steps] > 1.01 )
                {
                    ans.pb(i);
                    int k = i;
                    rof(step_count , steps , 1)
                    {
                        ans.pb( parent[i][k][step_count] );
                        k = parent[i][k][step_count];
                    }
                    found = true;
                }
                if( found ) break;
