...
vi dfs_num , dfs_low , critical , dfs_parent;
int dfs_counter;
int dfs_head;
int root_child;

void critical_points(int u)
{
    ...
    for( auto v: adj[u] )
    {
        if( dfs_num[v] == -1 )
        {
            if( u == dfs_head )     root_child++;
            dfs_parent[v] = u;
            
            critical_points(v);

            if( dfs_low[v] >= dfs_num[u] )      critical[u] = 1;
